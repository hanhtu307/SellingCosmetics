...
// Truy vấn tất cả bản ghi từ bảng marquee
$stmt = $conn->prepare("SELECT content FROM marquees WHERE is_active = 1");
$stmt->execute();
$result = $stmt->get_result();
$marquee_contents = $result->fetch_all(MYSQLI_ASSOC);
$stmt->close();

$navbars = [];
$resultNavbars = $conn->query("SELECT text, url, icon FROM navbar WHERE is_active = 1 ORDER BY order_num ASC");
if ($resultNavbars) {
...
$featured_products = [];
$resultFeaturedProducts = $conn->query("SELECT id, name, product_image, price, old_price, rating, sold, location FROM products ORDER BY sold DESC LIMIT 12");
...
$resultBestSelling = $conn->query("SELECT id, name, product_image, price, old_price, rating, sold, location FROM products ORDER BY sold DESC LIMIT 3");
if ($resultBestSelling) {
    while ($row = $resultBestSelling->fetch_assoc()) {
        $best_selling_products[] = $row;
    }
} else {
    error_log("SQL Error (best_selling_products): " . $conn->error, 3, "errors.log");
}
...
<body>
    ...
    <header>
        ...
        <div class="topbar">
            <a href="home.php" class="logo">
                <img src="assets/images/logo1.png" alt="Luna Beauty" style="height: 140px; margin-right: 80px;">
            </a>
        ...
        <nav class="navbar">
            <?php if (!empty($navbars)): ?>
                <?php foreach ($navbars as $navbar): ?>
                    <a href="<?php echo htmlspecialchars($navbar['url']); ?>"
                        <?php echo (strpos($navbar['url'], '#') === 0) ? 'onclick="' . htmlspecialchars($navbar['text']) . '()"' : ''; ?>>
                        <?php if (!empty($navbar['icon'])): ?>
                            <i class="<?php echo htmlspecialchars($navbar['icon']); ?>"></i>
                        <?php endif; ?>
                        <?php echo htmlspecialchars($navbar['text']); ?>
                    </a>
                <?php endforeach; ?>
            <?php else: ?>
                <!-- Mặc định nếu không có dữ liệu từ cơ sở dữ liệu -->
    ...
    <div class="marquee-container">
        <div class="marquee-text">
            <?php
            if (!empty($marquee_contents)) {
                foreach ($marquee_contents as $content) {
                    echo '<span>' . htmlspecialchars($content['content']) . '</span> ';
                }
            } else {
                echo '<span>🌟 Chào mừng bạn đến với Luna Beauty! 🌟</span>';
            }
            ?>
        </div>
    </div>
